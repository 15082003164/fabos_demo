...
$(document).on("click",".templatemo-edit-btn",function () {
    ...
    var upKindsNum = $(this).parents("tr").find("td:eq(2)").text();

    $("#kinds_id").text(upKindsid);
    $("#kinds_name").text(upKindsname);
...
$(document).on("click","#saveUpdate",function () {
    ...
    var ukindsname = $("#kinds_name").text();
    var ukindsnum = $("#kinds_num").val();


    $.ajax({
        url:"/shop/cooker/kinds/update/",
        type:"POST",
        data:{
            kinds_id:ukindsid,
...
function to_page(path, page) {
    $.ajax({
        url: path + "/cooker/kinds/showjson",
        data: "page=" + page,
        type: "get",
        success: function (result) {
...
function build_goods_table(path,result) {
    ...
    $.each(kinds, function (index,item) {
        ...
        $("<tr></tr>").append(kindsid).append(kindsname).append(kindsnum).append(editTd).appendTo("#kindsinfo tbody");
    })
}
